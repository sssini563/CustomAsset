    ...
    public function store(AssetCheckoutRequest $request, $assetId) : RedirectResponse
    {
        ...
        try {
            ...
            if ($asset->checkOut($target, $admin, $checkout_at, $expected_checkin, $request->get('note'), $request->get('name'))) {
                // If user requested document creation, locate the most recent checkout action log and flag it
                $createDoc = $request->boolean('checkout_and_create_document');
                if ($createDoc) {
                    /** @var Actionlog|null $actionLog */
                    $actionLog = Actionlog::where('item_type', get_class($asset))
                        ->where('item_id', $asset->id)
                        ->where('action_type','checkout')
                        ->orderByDesc('id')
                        ->first();
                    if ($actionLog) {
                        $actionLog->create_document = true;
                        $actionLog->save();
                        // Generate document (ignore failures silently to not block checkout)
                        try {
                            DocumentCreator::fromAssetCheckout($asset, $actionLog, $admin ?? auth()->user(), true);
                        } catch (\Throwable $e) {
                            \Log::error('Document creation failed: '.$e->getMessage());
                        }
                }
                }
                return Helper::getRedirectOption($request, $asset->id, 'Assets')->with('success', trans('admin/hardware/message.checkout.success'));
            }

...
class LoginController extends Controller
{
    ...
    /**
     * Internal profiling logger (enabled when env LOGIN_PROFILING=true)
     */
    protected function profLog(string $label, float $start): void
    {
        if (!env('LOGIN_PROFILING')) { return; }
        $ms = round((microtime(true) - $start) * 1000, 1);
        Log::info("LOGIN_PROF | {$label} = {$ms}ms");
    }
    ...
    $tValidate = microtime(true);
    $validator = $this->validator($request->all());
    $this->profLog('validate', $tValidate);
        ...
        $tThrottle = microtime(true);
        if ($lockedOut = $this->hasTooManyLoginAttempts($request)) {
            $this->fireLockoutEvent($request);

            return $this->sendLockoutResponse($request);
        }
        $this->profLog('throttle-check', $tThrottle);

    $user = null;
    $inputUsername = $request->input('username');
    // Detect whether this is a local user; if so, prefer local auth first to avoid slow LDAP path
    $existingUser = User::where('username', '=', $inputUsername)->whereNull('deleted_at')->first();
    $preferLocalFirst = $existingUser && ((int)($existingUser->ldap_import) !== 1);

        // If this appears to be a local (non-LDAP) user, try local auth first for speed
        if ($preferLocalFirst) {
            Log::debug('Detected local user account; attempting local authentication first.');
            $tLocalA = microtime(true);
            if (Auth::attempt(['username' => $inputUsername, 'password' => $request->input('password'), 'activated' => 1], $request->input('remember'))) {
                $this->profLog('local-auth-attempt (preferred)', $tLocalA);
                $user = auth()->user();
            } else {
                $this->profLog('local-auth-attempt (preferred) FAILED', $tLocalA);
            }
        }

        // If still not authenticated, and LDAP is enabled, try LDAP next
        if (!$user && Setting::getSettings()->ldap_enabled) { // avoid hitting the $this->ldap
            LOG::debug('LDAP is enabled. Attempting LDAP authentication.');
            try {
                $tLdap = microtime(true);
                $user = $this->loginViaLdap($request);
                Auth::login($user, $request->input('remember'));
                $this->profLog('ldap-login-total', $tLdap);
            } catch (\Exception $e) {
        ...
        if (! $user) {
            ...
            $this->profLog('local-auth-attempt', $tLocal);
        }

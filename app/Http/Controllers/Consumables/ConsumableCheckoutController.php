...
class ConsumableCheckoutController extends Controller
{
    ...
    public function store(Request $request, $consumableId)
    {
        ...
        // Optional: create document on checkout
        if ($request->boolean('checkout_and_create_document')) {
            try {
                DocumentCreator::fromConsumableCheckout($consumable, $user, auth()->user(), (int)$quantity, $request->input('note'), true);
            } catch (\Throwable $e) {
                \Log::error('Document creation for consumable failed: '.$e->getMessage());
            }
        }

...
class LicenseCheckoutController extends Controller
{
    ...
    public function store(LicenseCheckoutRequest $request, $licenseId, $seatId = null)
    {
        ...
        if ($request->filled('asset_id')) {
            ...
            // Optional document creation when checking out to asset
            if ($checkoutTarget && $request->boolean('checkout_and_create_document')) {
                try {
                    DocumentCreator::fromLicenseCheckout($licenseSeat, $checkoutTarget, auth()->user(), $request->input('notes'), true);
                } catch (\Throwable $e) {
                    Log::error('Document creation for license->asset failed: '.$e->getMessage());
                }
            }
            ...
            // Optional document creation when checking out to user
            if ($checkoutTarget && $request->boolean('checkout_and_create_document')) {
                try {
                    DocumentCreator::fromLicenseCheckout($licenseSeat, $checkoutTarget, auth()->user(), $request->input('notes'), true);
                } catch (\Throwable $e) {
                    Log::error('Document creation for license->user failed: '.$e->getMessage());
                }
            }
        }

...
class SettingsController extends Controller
{
...

    {
        ...
        // Documents: Public token expiry days
        if ($request->filled('document_public_token_days')) {
            $days = (int) $request->input('document_public_token_days');
            $setting->document_public_token_days = $days >= 0 ? $days : null; // 0 or null means no expiry
        }
    ...
    public function postBranding(ImageUploadRequest $request) : RedirectResponse
    {
        ...
        if (!config('app.lock_passwords')) {
            ...
            $setting = $request->handleImages($setting, 1600, 'login_background', '', 'login_background');
            if ($request->input('clear_login_background') == '1') {
                $setting = $request->deleteExistingImage($setting, '', 'login_background');

...
class Ldap extends Model
{
    ...
    public static function connectToLdap()
    {
        ...
        $netTimeout = (int) (env('LDAP_NETWORK_TIMEOUT', 20));
    ...
    ldap_set_option($connection, LDAP_OPT_NETWORK_TIMEOUT, $netTimeout);
    if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | ldap_connect '.round((microtime(true)-$tConn)*1000,1).'ms, timeout='.$netTimeout.'s'); }

        if ($ldap_use_tls=='1') {
            $tTls = microtime(true);
            ldap_start_tls($connection);
            if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | ldap_start_tls '.round((microtime(true)-$tTls)*1000,1).'ms'); }
        }


        if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | ldap_connectToLdap TOTAL '.round((microtime(true)-$tAll)*1000,1).'ms'); }
        return $connection;
    }
    ...
    public static function findAndBindUserLdap($username, $password)
    {
        ...
        // Prioritize ldap_filter if it contains %s placeholder (for AD: sAMAccountName=%s)
        if (!empty($filter) && strpos($filter, '%s') !== false) {
            // Replace %s placeholder with username
            $filter = str_replace('%s', $username, $filter);
            $filterQuery = "({$filter})";
            Log::debug('Using ldap_filter: '.$filterQuery);
        } else {
            // Fallback to ldap_auth_filter_query (for OpenLDAP: uid=)
            $filterQuery = $settings->ldap_auth_filter_query.$username;
            $filterQuery = "({$filterQuery})";
            Log::debug('Using ldap_auth_filter_query: '.$filterQuery);
        }

        $tBind = microtime(true);
        ...
        if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | ldap_bind(user) '.round((microtime(true)-$tBind)*1000,1).'ms'); }
        $tSearch = microtime(true);
        if (! $results = ldap_search($connection, $baseDn, $filterQuery)) {
            throw new Exception('Could not search LDAP: ');
        }
        if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | ldap_search '.round((microtime(true)-$tSearch)*1000,1).'ms'); }
        ...
        $result = array_change_key_case($user);
        if (env('LOGIN_PROFILING')) { Log::info('LOGIN_PROF | findAndBindUserLdap TOTAL '.round((microtime(true)-$tAll)*1000,1).'ms'); }
        return $result;
    }

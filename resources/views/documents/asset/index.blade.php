  ...
  <div class="box-body">
    ...
    <table class="table table-striped table-bordered">
      ...
      <tbody>
        @forelse($documents as $doc)
          <tr>
            ...
            <td style="white-space:nowrap;">
              ...
              @php(
                $sig = $doc->relationLoaded('signatures') ? $doc->signatures : collect()
              )
              @php(
                $nonLegacy = $sig->reject(fn($s)=> in_array($s->role,['it_manager','atasan_penerima']))
              )
              @php(
                $assigned = $nonLegacy->filter(fn($s)=> !is_null($s->user_id))
              )
              @php(
                $canComplete = (!$isComplete) && $assigned->count() > 0 && ($assigned->where('status','signed')->count() === $assigned->count())
              )
        ...
        @empty
          <tr><td colspan="8" class="text-center">Tidak ada dokumen.</td></tr>
        @endforelse
      </tbody>
    </table>
    {{ $documents->links() }}
...
(function(){
  ...
  document.addEventListener('click',function(e){
      ...
      (async function(){
        ...
        try{
          ...
          if(action==='regen-token'){
            ...
            const copyBtn = tr ? tr.querySelector('[data-action="copy-link"]') : null;
    ...
    if(e.target.closest('[data-delete]')){
      var del=e.target.closest('[data-delete]'); if(del.disabled || del.classList.contains('disabled')) return; if(!confirm('Delete this document?')) return;
      var id=del.getAttribute('data-delete');
  fetch('/documents/'+id,{method:'DELETE',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}})
    ...
    if(e.target.closest('[data-lock]')){
      ...
      var orig = lockBtn.innerHTML; lockBtn.disabled = true; lockBtn.innerHTML = '<i class="fa fa-spinner fa-spin"></i>';
  fetch('/documents/'+id+'/complete', {method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}})
        .then(async r=>{ if(!r.ok){ let m='Lock failed'; try{const j=await r.json(); if(j.message) m=j.message;}catch(_){} throw new Error(m);} return r.json(); })
        .then(()=>{ window.location.href = BASE_URL + '/documents/'+id+'/pdf'; })
        .catch(err=>{ alert(err.message||'Error'); lockBtn.disabled=false; lockBtn.innerHTML=orig; });
      return;
    }
    if(e.target.closest('[data-complete]')){
      var btn=e.target.closest('[data-complete]'); if(btn.disabled) return;
      if(!confirm('Mark this document as COMPLETE and lock it from further edits?')) return;
      var id=btn.getAttribute('data-complete');
      var origHtml=btn.innerHTML; btn.disabled=true; btn.innerHTML='<i class="fa fa-spinner fa-spin"></i>';
  fetch('/documents/'+id+'/complete',{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}})
        .then(async r=>{
          if(!r.ok){
            let msg='Failed to complete';
            try{ const j=await r.json(); if(j && j.message) msg=j.message; }catch(_e){}
            throw new Error(msg);
          }
          return r.json();
        })
        .then(()=> location.reload())
        .catch(err=>{ alert(err.message||'Error'); btn.disabled=false; btn.innerHTML=origHtml; })
    }
  });

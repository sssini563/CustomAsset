<div class="modal-header">
  ...
  <h4 class="modal-title">Approval Document {{ $document->document_number }}</h4>
</div>
...
(function(){
  ...
  let r = await fetch('/documents/'+id+'/signature/'+role+'/disable-token',{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}});
      ...
      if (e.name === 'TypeError') {
        alert('Gagal memanggil server (network error). Pastikan Anda masih login dan URL sesuai.');
      } else {

...
@section('content')
    <div class="box box-default">
        <div class="box-header with-border">
            ...
            <div class="box-tools">
                <a href="{{ route('documents.print', $document->id) }}" target="_blank" class="btn btn-default" title="Cetak"><i
                        class="fa fa-print"></i></a>
                @can('update', $document)
                    <button class="btn btn-default" id="btn-recompute" title="Hitung Ulang Status"><i
                            class="fa fa-refresh"></i></button>
                    @php
                        $legacy = ['it_manager', 'atasan_penerima'];
                        $assignedAll = $document->signatures
                            ->reject(fn($s) => in_array($s->role, $legacy))
                            ->filter(fn($s) => !is_null($s->user_id));
                        $canComplete =
                            in_array($document->overall_status, ['pending', 'complete_sign']) &&
                            $assignedAll->count() > 0 &&
                            $assignedAll->where('status', 'signed')->count() === $assignedAll->count();
                    @endphp
                    @if ($canComplete)
                        <button class="btn btn-success" id="btn-complete" title="Kunci &amp; Buat PDF"><i
                                class="fa fa-lock"></i></button>
                    @endif
                @endcan
                @if (auth()->user()->isSuperUser() && $document->overall_status === 'pending')
                    <a href="{{ route('documents.edit', $document->id) }}" class="btn btn-primary" title="Ubah"><i
            ...
            <div class="tab-content" style="margin-top:15px;">
                <div class="tab-pane active" id="detail">
                    ...
                    <table class="table table-bordered table-condensed">
                        <tr>
                            ...
                            <td style="width:32%">{{ $document->document_number }}</td>
                            <th style="width:18%">Organization Structure</th>
                            <td style="width:32%">{{ $document->organization_structure ?: '' }}</td>
                        </tr>
                        <tr>
                    ...
                    <table class="table table-bordered table-condensed">
                        ...
                        <tr>
                            ...
                            <td>{{ $document->atasan_penerima_name ?: '' }}</td>
                        </tr>
                        <tr>
                            <th>Asset Number</th>
                    ...
                    <table class="table table-bordered table-condensed">
                        <tr>
                            ...
                            <td style="width:82%">{{ $document->device_name ?: '' }}</td>
                        </tr>
                        <tr>
                            <th>Merk</th>
                            <td>{{ $document->merk ?: '' }}</td>
                    ...
                    <table class="table table-bordered table-condensed">
                        @if (strtolower($document->type) === 'license')
                            <tr>
                                ...
                                <td style="width:82%">{{ $document->license_key ?: '' }}</td>
                            </tr>
                            <tr>
                                <th>Seats</th>
                        ...
                        @elseif(strtolower($document->type) === 'accessory')
                            <tr>
                                ...
                                <td style="width:82%">{{ $document->accessory_part_number ?: '' }}</td>
                            </tr>
                            <tr>
                                <th>Serial Number</th>
                        ...
                        @elseif(strtolower($document->type) === 'component')
                            <tr>
                                ...
                                <td style="width:82%">{{ $document->component_model ?: '' }}</td>
                            </tr>
                            <tr>
                                <th>Part No</th>
                        ...
                        @elseif(strtolower($document->type) === 'consumable')
                            <tr>
                                ...
                                <td style="width:82%">{{ $document->consumable_batch ?: '' }}</td>
                            </tr>
                            <tr>
                                <th>Qty</th>
                        ...
                        @else
                            <tr>
                                <th style="width:18%">Windows</th>
                                <td style="width:82%">{{ $document->windows ?: '' }}</td>
                            </tr>
                            <tr>
                                <th>Office</th>
                    ...
                    <table class="table table-bordered table-condensed">
                        <tr>
                            <th style="width:18%">Dokumen Pengembalian Asset</th>
                            <td style="width:82%">{{ $document->dokumen_pengembalian_asset ?: '' }}</td>
                        </tr>
                        <tr>
                            <th>Dokumen Surat Pernyataan</th>
                            <td>{{ $document->dokumen_surat_pernyataan ?: '' }}</td>
                        </tr>
                        <tr>
                            <th>Catatan</th>
                    ...
                    <table class="table table-bordered table-condensed">
                        <tr>
                            ...
                            @php($d = is_array($s->document_defaults_asset_form ?? null) ? $s->document_defaults_asset_form : [])
                            <th style="width:18%">Jenis Dokumen</th>
                            <td style="width:32%">
                                {{ $d['jenis_dokumen'] ?? ($s->document_default_jenis_dokumen ?? 'Formulir') }}</td>
                            <th style="width:18%">Halaman</th>
                            <td style="width:32%">{{ $d['sp_hal'] ?? ($s->document_default_sp_hal ?? '1 dari 1') }}</td>
                        </tr>
                        <tr>
                            <th>Pemilik Proses</th>
                            <td>{{ $d['pemilik_proses'] ?? ($s->document_default_pemilik_proses ?? 'IT Business Support') }}
                            </td>
                            <th>No. Dokumen</th>
                            <td>{{ $document->doc_control_no ?: ($document->document_no ?: '') }}</td>
                        </tr>
                        <tr>
                            <th>Tanggal Efektif</th>
                            <td>{{ isset($d['effective_doc']) && $d['effective_doc'] ? \Carbon\Carbon::parse($d['effective_doc'])->format('d M Y') : '' }}
                            </td>
                            <th>Tgl. Peninjauan</th>
                            <td>{{ isset($d['revision_date']) && $d['revision_date'] ? \Carbon\Carbon::parse($d['revision_date'])->format('d M Y') : '' }}
                            </td>
                        </tr>
                        <tr>
                            <th>Proses Bisnis</th>
                            <td>{{ $d['proses_bisnis'] ?? ($s->document_default_proses_bisnis ?? 'Authorization Request') }}
                            </td>
                            <th>Petahana</th>
                            <td>{{ $d['author_doc'] ?? '' }}</td>
                        </tr>
                ...
                <div class="tab-pane" id="approval">
                    ...
                    <div class="alert alert-info" style="margin-bottom:10px;">
                        ...
                        <span>assigned: {{ $assigned->count() }}, signed:
                            {{ $assigned->where('status', 'signed')->count() }}, pending: {{ $pendingAssigned->count() }},
                            rejected: {{ $rejectedAssigned->count() }}.</span>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($document->signatures->sortBy('ordering') as $sig)
                                ...
                                @endif
                                @if ($sig->role === 'creator')
                                    @php $displayRole = 'Creator'; @endphp
                                @endif
                                @if ($sig->role === 'user')
                                    @php $displayRole = 'User'; @endphp
                                @endif
                                <tr>
                                    <td>{{ $displayRole }}</td>
                                    ...
                                    <td>
                                        @if ($sig->status == 'pending' && (!$sig->user_id || $sig->user_id == auth()->id()))
                                            <button class="btn btn-xs btn-success" data-action="sign"
                                                data-role="{{ $sig->role }}"
                                                data-id="{{ $document->id }}">Approve</button>
                                            <button class="btn btn-xs btn-danger" data-action="reject"
                                                data-role="{{ $sig->role }}"
                                                data-id="{{ $document->id }}">Reject</button>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="tab-pane" id="preview" style="height:calc(100vh - 250px);">
                    <iframe src="{{ route('documents.print', $document->id) }}"
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest && e.target.closest('#btn-recompute')) {
                ...
                fetch(BASE_URL + '/documents/{{ $document->id }}' + '/recompute', {
                        method: 'POST',
                        headers: {
            ...
            if (e.target.closest && e.target.closest('#btn-complete')) {
                ...
                btn.disabled = true;
                btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i>';
                fetch(BASE_URL + '/documents/{{ $document->id }}' + '/complete', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
            ...
            if (e.target.matches('[data-action]')) {
                ...
                fetch(BASE_URL + '/documents/' + id + '/' + action + '/' + role, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'

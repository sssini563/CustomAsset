                            ...
                            @if ($progress != -1)
                                <div class="col-md-10 col-sm-5 col-xs-12" style="height: 35px;" id='progress-container'>
                                    <div class="progress progress-striped-active" style="height: 100%;">
                                        <div id='progress-bar' class="progress-bar {{ $progress_bar_class }}"
                            ...
                            <div class="col-md-4 col-sm-5 col-xs-12 text-right pull-right">
                                ...
                                @if (!config('app.lock_passwords'))
                                    <label for="fileupload" class="btn btn-primary"
                                        ...
                                        <i class="fa fa-upload"></i> <span>{{ trans('button.select_file') }}</span>
                                    </label>
                                    <!-- The file input field used as target for the file upload widget -->
                                    <input id="fileupload" type="file" name="files[]"
                                        data-url="{{ route('api.imports.index') }}" accept="text/csv"
                            ...
                            <table data-id-table="upload-table" data-side-pagination="client" id="upload-table"
                                ...
                                @foreach ($this->files as $currentFile)
                                    ...
                                    <tr style="{{ $this->activeFile && $currentFile->id == $this->activeFile->id ? 'font-weight: bold' : '' }}"
                                        class="{{ $this->activeFile && $currentFile->id == $this->activeFile->id ? 'warning' : '' }}">
                                        <td>{{ $currentFile->file_path }}</td>
                                        <td>{{ Helper::getFormattedDateObject($currentFile->created_at, 'datetime', false) }}
                                        </td>
                                        <td>{{ $currentFile->adminuser ? $currentFile->adminuser->present()->fullName : '--' }}
                                        </td>
                                        <td>{{ Helper::formatFilesizeUnits($currentFile->filesize) }}</td>
                                        <td class="col-md-1 text-right" style="white-space: nowrap;">
                                            <button class="btn btn-sm btn-info"
                                                wire:click="selectFile({{ $currentFile->id }})" data-tooltip="true"
                                                title="{{ trans('general.import_this_file') }}">
                                                <i class="fa-solid fa-list-check" aria-hidden="true"></i>
                                                <span class="sr-only">{{ trans('general.import') }}</span>
                                            </button>
                                            <a href="#" wire:click.prevent="$set('activeFileId',null)">
                                                <button class="btn btn-sm btn-danger"
                                                    wire:click="destroy({{ $currentFile->id }})">
                                            ...
                                            <td colspan="5">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-xs-12">
                                                        <x-input.select name="typeOfImport" id="import_type"
                                                            :options="$importTypes" :selected="$typeOfImport" :for-livewire="true"
                                                            :include-empty="true" :data-placeholder="trans('general.select_var', [
                                                                'thing' => trans('general.import_type'),
                                                            ])" {{-- placeholder needed so that the form-helper will put an empty option first --}}
                                                            placeholder="" {{-- Remove this if the list gets long enough that we need to search --}}
                                                ...
                                                <div class="form-group col-md-9 col-md-offset-3">
                                                    ...
                                                    @if ($typeOfImport === 'user')
                                                        ...
                                                        <p class="help-block">
                                                            {{ trans('general.send_welcome_email_import_help') }}</p>
                                                    @endif
                                                ...
                                                @if ($statusText)
                                                    <div class="alert col-md-8 col-md-offset-3{{ $statusType == 'success' ? ' alert-success' : ($statusType == 'error' ? ' alert-danger' : ' alert-info') }}"
                                                ...
                                                @if ($typeOfImport)
                                                    ...
                                                    @if (!empty($headerRow))
                                                        @foreach ($headerRow as $index => $header)
                                                            <div class="form-group col-md-12"
                                                                wire:key="header-row-{{ $index }}">

                                                                <label for="field_map.{{ $index }}"
                                                                    class="col-md-3 control-label text-right">{{ $header }}</label>
                                                                <div class="col-md-4">
                                                                    <x-input.select :name="'field_map.' . $index" :for-livewire="true"
                                                                        :placeholder="trans(
                                                    ...
                                                    <div class="form-group col-md-12">
                                                        <div class="col-md-3 text-left">
                                                            <a href="#"
                                                                wire:click.prevent="$set('activeFileId',null)">{{ trans('general.cancel') }}</a>
                                                        </div>
                                                        <div class="col-md-9">
                                                            <button type="submit" class="btn btn-primary col-md-5"
                                                                id="import">{{ trans('admin/hardware/message.import.import_button') }}</button>
                                                            <br><br>
                                                        </div>
                                                    </div>
    ...
    <script>
                    ...
                    $('#fileupload').fileupload({
                        dataType: 'json',
                            ...
                            $wire.$set('progress_message',
                                '<i class="fas fa-check faa-pulse animated"></i> {{ trans('general.notification_success') }}'
                            );
                        ...
                        fail: function() {
                            ...
                            $wire.$set('progress', 100);
                            $wire.$set('progress_message',
                                '<i class="fas fa-exclamation-triangle faa-pulse animated"></i> {{ trans('general.upload_error') }}'
                            );
                        }
                    });
                }
        ...
        $(function() {
            ...
            $('#upload-table').on('click', '#import', function() {
                ...
                $wire.$set('statusText',
                    '<i class="fa fa-spinner fa-spin" aria-hidden="true"></i> {{ trans('admin/hardware/form.processing_spinner') }}'
                );
                $wire.generate_field_map().then(function(mappings_raw) {
                    ...
                    $.post({
                        ...
                        $wire.$set('statusText',
                            "{{ trans('general.success_redirecting') }}");
                        // console.dir(body)
                        ...
                        if ((body) && (body.status) && body.status == 'import-errors') {
                            ...
                            if ((body) && (error in body)) {
                                ...
                                $wire.$set('message',
                                    "{{ trans('general.importer_generic_error') }}");
                            }

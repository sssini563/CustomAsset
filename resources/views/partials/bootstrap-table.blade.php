    ...
    <script nonce="{{ csrf_token() }}">
        ...
        function hardwareAuditFormatter(value, row) {
            return '<a href="{{ config('app.url') }}/hardware/' + row.id +
                '/audit" class="actions btn btn-sm btn-primary" style="background-color:#27ae60 !important;border-color:#27ae60 !important;" data-tooltip="true" title="{{ trans('general.audit') }}"><x-icon type="audit" /><span class="sr-only">{{ trans('general.audit') }}</span></a>&nbsp;';
        }
            ...
            return function(value, row) {
                ...
                if ((row.available_actions) && (row.available_actions.clone === true)) {
                    actions += '<a href="{{ config('app.url') }}/' + dest + '/' + row.id +
                        '/clone" class="actions btn btn-sm btn-info" style="background-color:#5dade2 !important;border-color:#5dade2 !important;" data-tooltip="true" title="{{ trans('general.clone_item') }}"><x-icon type="clone" /><span class="sr-only">{{ trans('general.clone_item') }}</span></a>&nbsp;';
                }
                ...
                if ((row.available_actions) && (row.available_actions.update === true)) {
                    actions += '<a href="{{ config('app.url') }}/' + dest + '/' + row.id +
                        '/edit" class="actions btn btn-sm btn-warning" style="background-color:#f39c12 !important;border-color:#f39c12 !important;" data-tooltip="true" title="{{ trans('general.update') }}"><x-icon type="edit" /><span class="sr-only">{{ trans('general.update') }}</span></a>&nbsp;';
                } else {
                    if ((row.available_actions) && (row.available_actions.update != true)) {
                        actions +=

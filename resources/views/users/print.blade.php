    ...
    @if ($snipeSettings->brand == '3')
        ...
        <h2>
            @if ($snipeSettings->logo != '')
                <img class="print-logo" src="{{ Storage::disk('public')->url($snipeSettings->logo) }}">
            @endif
...
@foreach ($users as $show_user)
    ...
    <p></p>{{ trans('admin/users/general.all_assigned_list_generation') }}
    {{ Helper::getFormattedDateObject(now(), 'datetime', false) }}
    ...
    @if ($show_user->assets->count() > 0)
        ...
        <table class="snipe-table table table-striped inventory" id="AssetsAssigned" data-pagination="false"
            data-id-table="AssetsAssigned" data-search="false" data-side-pagination="client" data-sortable="true"
            data-toolbar="#assets-toolbar" data-sort-order="desc" data-sort-name="created_at"
            data-show-columns="true" data-cookie-id-table="AssetsAssigned">
            <thead>
                <th data-field="asset_id" data-sortable="false" data-visible="true" data-switchable="false">#</th>
                <th data-field="asset_image" data-sortable="true" data-visible="false" data-switchable="true">
                    {{ trans('general.image') }}</th>
                <th data-field="asset_tag" data-sortable="true" data-visible="true" data-switchable="false">
                    {{ trans('admin/hardware/table.asset_tag') }}</th>
                <th data-field="asset_name" data-sortable="true" data-visible="true">{{ trans('general.name') }}
                </th>
                <th data-field="asset_category" data-sortable="true" data-visible="true">
                    {{ trans('general.category') }}</th>
                <th data-field="asset_model" data-sortable="true" data-visible="true">
                    {{ trans('admin/hardware/form.model') }}</th>
                <th data-field="rtd_location" data-sortable="true" data-visible="true">
                    {{ trans('admin/hardware/form.default_location') }}</th>
                <th data-field="asset_location" data-sortable="true" data-visible="false">
                    {{ trans('general.location') }}</th>
                <th data-field="asset_serial" data-sortable="true" data-visible="true">
                    {{ trans('admin/hardware/form.serial') }}</th>
                <th data-field="asset_checkout_date" data-sortable="true" data-visible="true">
                    {{ trans('admin/hardware/table.checkout_date') }}</th>
                <th data-field="signature" data-sortable="false" data-visible="true">
                    {{ trans('general.signature') }}</th>
            </thead>
            <tbody>
                @foreach ($show_user->assets as $asset)
                    @php
                        if ($asset->model->category && $asset->model->category->getEula()) {
                    ...
                    <tr>
                        ...
                        <td>{{ $asset->name }}</td>
                        <td>{{ $asset->model && $asset->model->category ? $asset->model->category->name : trans('general.invalid_category') }}
                        </td>
                        ...
                        <td>
                            @if ($asset->getLatestSignedAcceptance($show_user))
                                <img style="width:auto;height:100px;"
                                    src="{{ asset('/') }}display-sig/{{ $asset->getLatestSignedAcceptance($show_user)->accept_signature }}">
                            @endif
                        </td>
                    </tr>
                    @if ($settings->show_assigned_assets)
                        ...
                        @foreach ($asset->assignedAssets as $asset)
                            <tr>
                                ...
                                <td>
                                    @if ($asset->getImageUrl())
                                        <img src="{{ $asset->getImageUrl() }}" class="thumbnail"
                                ...
                                <td>{{ $asset->model ? $asset->model->name : trans('general.invalid_model') }}
                                </td>
                                <td>{{ $asset->defaultLoc ? $asset->defaultLoc->name : '' }}</td>
                                <td>{{ $asset->location ? $asset->location->name : '' }}</td>
                                <td>{{ $asset->serial }}</td>
    ...
    @if ($show_user->licenses->count() > 0)
        <div id="licenses-toolbar">
            <h4>{{ trans_choice('general.countable.licenses', $show_user->licenses->count(), ['count' => $show_user->licenses->count()]) }}
            </h4>
        </div>

        <table class="snipe-table table table-striped inventory" id="licensessAssigned"
            data-toolbar="#licenses-toolbar" data-pagination="false" data-id-table="licensessAssigned"
            data-search="false" data-side-pagination="client" data-sortable="true" data-sort-order="desc"
            data-sort-name="created_at" data-show-columns="true" data-cookie-id-table="licensessAssigned">
            <thead>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        @if ($license->getLatestSignedAcceptance($show_user))
                            <img style="width:auto;height:100px;"
                                src="{{ asset('/') }}display-sig/{{ $license->getLatestSignedAcceptance($show_user)->accept_signature }}">
                        @endif
                    </td>
                </tr>
                @php
    ...
    @if ($show_user->accessories->count() > 0)
        <div id="accessories-toolbar">
            <h4>{{ trans_choice('general.countable.accessories', $show_user->accessories->count(), ['count' => $show_user->accessories->count()]) }}
            </h4>
        </div>

        <table class="snipe-table table table-striped inventory" id="accessoriesAssigned"
            data-toolbar="#accessories-toolbar" data-pagination="false" data-id-table="accessoriesAssigned"
            data-search="false" data-side-pagination="client" data-sortable="true" data-sort-order="desc"
            data-sort-name="created_at" data-show-columns="true" data-cookie-id-table="accessoriesAssigned">
            <thead>
                <tr>
                    <th style="width: 20px;" data-sortable="false" data-switchable="false">#</th>
                    <th data-field="accessory_image" data-sortable="true" data-visible="true">
                        {{ trans('general.image') }}</th>
                    <th style="width: 40%;" data-sortable="true" data-switchable="false">
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($accessory->getImageUrl())
                                <img src="{{ $accessory->getImageUrl() }}" class="thumbnail"
                                    style="max-height: 50px;">
                            @endif
                        </td>
                        <td>{{ $accessory->manufacturer ? $accessory->manufacturer->name : '' }}
                            {{ $accessory->name }} {{ $accessory->model_number }}</td>
                        ...
                        <td>
                            @if ($accessory->getLatestSignedAcceptance($show_user))
                                <img style="width:auto;height:100px;"
                                    src="{{ asset('/') }}display-sig/{{ $accessory->getLatestSignedAcceptance($show_user)->accept_signature }}">
                            @endif
                        </td>
                    </tr>
                    @php
    ...
    @if ($show_user->consumables->count() > 0)
        <div id="consumables-toolbar">
            <h4>{{ trans_choice('general.countable.consumables', $show_user->consumables->count(), ['count' => $show_user->consumables->count()]) }}
            </h4>
        </div>

        <table class="snipe-table table table-striped inventory" id="consumablesAssigned" data-pagination="false"
            data-toolbar="#consumables-toolbar" data-id-table="consumablesAssigned" data-search="false"
            data-side-pagination="client" data-sortable="true" data-sort-order="desc"
            data-sort-name="created_at" data-show-columns="true" data-cookie-id-table="consumablesAssigned">
            <thead>
                <tr>
                    <th style="width: 20px;" data-sortable="false" data-switchable="false"></th>
                    <th style="width: 40%;" data-sortable="true" data-switchable="false">
            ...
            @foreach ($show_user->consumables as $consumable)
                @if ($consumable)
                    ...
                    <tr>
                        ...
                        <td>{{ $consumable->manufacturer ? $consumable->manufacturer->name : '' }}
                            {{ $consumable->name }} {{ $consumable->model_number }}</td>
                    @else
                        {{ $consumable->manufacturer ? $consumable->manufacturer->name : '' }}
                        {{ $consumable->name }} {{ $consumable->model_number }}
                @endif
                </td>
                ...
                <td>
                    @if ($consumable->getLatestSignedAcceptance($show_user))
                        <img style="width:auto;height:100px;"
                            src="{{ asset('/') }}display-sig/{{ $consumable->getLatestSignedAcceptance($show_user)->accept_signature }}">
                    @endif
                </td>
                </tr>
                @php
